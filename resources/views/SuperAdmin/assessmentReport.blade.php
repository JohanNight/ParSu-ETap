...
<div class="flex justify-end mt-5">
    <a href="{{ route('generatePDF') }}" class="text-lg SemiB-font px-4 py-2 bg-green-500 rounded-md mr-10">PDF</a>
</div>
            ...
            @foreach ($getTotalServiceAvail as $serviceTitle => $count)
                <tr class="hover:bg-grey-lighter text-center">
                    <td class="py-2 px-4 border-b border-grey-light text-[13px] SemiB-font">
                        {{ $serviceTitle }}</td>
                    <td class="py-2 px-4 border-b border-grey-light text-[15px] Regular-font">
                        {{ $count }}</td>
                    <td class="py-2 px-4 border-b border-grey-light Regular-font">{{ $count }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
