                    ...
                    <div class="mt-2 mb-3 gap-2 flex flex-col border-2 border-black p-1 rounded-md">
                        ...
                        @foreach ($ccInstruction as $Instruction)
                            <textarea name="instruction" id="instruction" cols="30" rows="3"
                                class="w-full bg-gray-100 border-2 px-1 py-1 focus:outline-none text-sm" autocomplete="off">{{ $Instruction->instruction }}</textarea>
                        @endforeach

                    </div>

                    {{-- CC Number 0 --}}
                    @foreach ($ccQuestions as $index => $Question)
                        <div class="set-question w-full flex flex-col border-2 border-black p-2 mb-3 rounded-md">
                            ...
                            <div class="mt-2 mb-3 flex flex-col gap-4">
                                <div class="gap-2 flex flex-col">
                                    <label for="cc_questions[{{ $index }}][question]"
                                        class="text-[15px] Reg-font">Citizen Charter
                                        Question 1:
                                    </label>
                                    <input type="text" name="cc_questions[{{ $index }}][question]"
                                        id="cc_questions[{{ $index }}][question]"
                                        class="w-full h-12 bg-gray-100 border-2 px-1 py-1 focus:outline-none text-md Reg-font"
                                        autocomplete="off" value="{{ $Question->description }}">
                                </div>
                            </div>
                            <div class="mt-3">
                                ...
                                <div class="Option flex flex-col p-3 w-full gap-4 mt-4">
                                    @foreach ($Question->CcOption as $index => $option)
